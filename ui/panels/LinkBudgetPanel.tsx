...
export const LinkBudgetPanel: React.FC = () => {
    ...
    return (
        <div className="w-80 h-full flex flex-col tech-panel border-l border-cyan-900/50 bg-[#101c2e]/95 z-20 shadow-xl">
            <div className="p-3 border-b border-cyan-900/30 flex justify-between items-center bg-[#0b1220]">
                <h2 className="text-sm font-bold text-slate-200 flex items-center gap-2">
                    <Calculator size={14} className="text-cyan-500"/> QUỸ ĐƯỜNG TRUYỀN
                </h2>
            ...
            <div className="p-4 space-y-4 overflow-y-auto">
                ...
                <div className="space-y-2">
                    <label className="text-[10px] text-cyan-500 font-bold uppercase tracking-wider">THÔNG SỐ LIÊN KẾT</label>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full bg-[#0b1220] border border-slate-700 text-slate-300 text-xs p-1 rounded font-mono"
                            />
                        </div>
                        <div>
                        ...
                        <div>
                         ...
                         <div>
                            ...
                            <div className="text-xs text-cyan-400 font-bold py-1 bg-cyan-900/20 px-2 rounded border border-cyan-900/50">{frequencyGHz.toFixed(2)}</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    <label className="text-[10px] text-cyan-500 font-bold flex items-center gap-2 uppercase tracking-wider">
                        <Signal size={12}/> KẾT QUẢ TÍNH TOÁN
                    </label>

                    <div className="bg-[#0b1220] p-2 rounded border border-slate-700 space-y-1 shadow-sm">
                        <div className="flex justify-between text-xs border-b border-slate-800 pb-1">
                            ...
                            <span className="text-slate-300 font-mono font-bold">{stats.fspl.toFixed(2)} dB</span>
                        </div>
                        <div className="flex justify-between text-xs border-b border-slate-800 py-1">
                            ...
                            <span className={`${stats.rxPower > receiverSensitivitydBm ? 'text-emerald-400' : 'text-red-400'} font-mono font-bold`}>
                                {stats.rxPower.toFixed(2)} dBm
                            </span>
                        </div>
                        <div className="flex justify-between text-xs pt-1">
                            <span className="text-slate-500">SNR (Nhiệt)</span>
                            <span className="text-amber-500 font-mono font-bold">{stats.snr.toFixed(1)} dB</span>
                        </div>
                    </div>

                    <div className={`p-2 rounded border shadow-sm ${stats.isLinked ? 'bg-emerald-900/20 border-emerald-900/50' : 'bg-red-900/20 border-red-900/50'}`}>
                        <div className="flex justify-between items-center mb-1">
                            <span className="text-[10px] font-bold text-slate-400">DỰ TRỮ FADING (MARGIN)</span>
                            <Wifi size={14} className={stats.isLinked ? 'text-emerald-500' : 'text-red-500'}/>
                        </div>
                        <div className={`text-2xl font-mono font-bold text-center ${stats.isLinked ? 'text-emerald-400' : 'text-red-400'}`}>
